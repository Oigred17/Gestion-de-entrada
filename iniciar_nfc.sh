#!/bin/bash
# ============================================================
# COBAO NFC Reader — Lanzador Linux
#
# Estación de entrada (lector fisico).
# En esta carpeta solo necesitas:
#   - nfc_reader.py
#   - nfc_url.txt   (URL del backend .../api/v1/nfc/scan)
#   - nfc_key.txt   (misma llave que NFC_API_KEY del servidor)
#
# NO uses usuario ni contraseña aqui.
#
# Requisitos:
#   sudo apt install pcscd python3-pyscard python3-venv
#   pip install pyscard
# ============================================================
...
if ! python3 -c "import smartcard" 2>/dev/null; then
    echo "  Instalando pyscard..."
    pip3 install pyscard --quiet 2>/dev/null || pip install pyscard --quiet
fi
